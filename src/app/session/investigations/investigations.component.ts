...
export class InvestigationsComponent implements OnInit {
  ...
  initComplaintsForm() {
    ...
    this.formGroup.get('entry1').valueChanges.subscribe(value => {
      if(value !== '') {
        this.suggestions = this.apiservice.getInvestigationsSuggestions(value);
      }
    });
  ...
  uploadData() {
    ...
    this.spinner.show();
    if (this.edit) {
      this.apiservice.updateSessionInvestigations(this.notes.id,formData).subscribe(results => {
        this.spinner.hide();
        this.notes = results;
        this.edit = false;
      }, error => {
      ...
      this.apiservice.createSessionInvestigations(this.sessionID,formData).subscribe(results => {
        this.spinner.hide();
        this.notes = results;
        this.sectionCompleted = true;
        this.navToNext();
