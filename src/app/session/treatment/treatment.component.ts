...
export class TreatmentComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe(params => {
      if (params.sessionID !== undefined && params.sessionID !== null) {
        ...
        this.spinner.show();
        this.apiservice.getSessionTreatment(this.sessionID).subscribe(results => {
          this.spinner.hide();
          this.notes = results;
  ...
  initComplaintsForm() {
    ...
    this.formGroup.get('entry1').valueChanges.subscribe(value => {
      if(value !== '') {
        this.suggestions = this.apiservice.getTreatmentSuggestions(value);
      }
    });
  ...
  uploadData() {
    ...
    this.spinner.show();
    if (this.edit) {
      this.apiservice.updateSessionTreatment(this.notes.id,formData).subscribe(results => {
        this.spinner.hide();
        this.notes = results;
        this.edit = false;
      }, error => {
      ...
      this.apiservice.createSessionTreatment(this.sessionID,formData).subscribe(results => {
        this.spinner.hide();
        this.notes = results;
        this.sectionCompleted = true;
        this.navToNext();
